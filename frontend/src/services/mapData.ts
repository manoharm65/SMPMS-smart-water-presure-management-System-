...
export function parsePipelines(
  ...
  for (const [idx, f] of fc.features.entries()) {
    ...
    pipelines.push({ id, zoneId, path, properties: (f.properties ?? undefined) as unknown as Record<string, unknown> })
  }
  return pipelines
}
...
export function parseValves(fc: FeatureCollection<Point, GeoJsonProperties>): Valve[] {
  ...
  for (const [idx, f] of fc.features.entries()) {
    ...
    const name = getStringProp(f.properties, 'name') ?? getStringProp(f.properties, 'Name') ?? `Valve ${id}`
    const zoneId = getStringProp(f.properties, 'zoneId') ?? getStringProp(f.properties, 'dma') ?? 'DMA-UNKNOWN'
    valves.push({
      id,
      name,
      zoneId,
      position: toLatLngTuple(f.geometry.coordinates),
      properties: (f.properties ?? undefined) as unknown as Record<string, unknown>,
    })
  }
  return valves
}
